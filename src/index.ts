  ...
  for (const product of products) {
    ...
    if (!Number.isSafeInteger(quantity) || quantity <= 0)
      throw new RangeError(
        `${product.name}.quantity must be a positive integer`,
      );
    for (let index = 0; index < quantity; index++) {
      result.push({
        length: product.length,
        width: product.width,
        height: product.breadth,
      });
    }
  }
  return result;
...
function shelfPack(
  ...
  for (const rect of rectangles) {
    const orientations =
      rect.length === rect.width
        ...
        : [
            [rect.length, rect.width] as const,
            [rect.width, rect.length] as const,
          ];
    let best:
      | { shelf: number; length: number; width: number; score: number }
      | undefined;
    for (const [length, width] of orientations) {
      ...
      for (let shelf = 0; shelf <= shelves.length; shelf++) {
        ...
        const totalDepth =
          shelves.reduce(
            (sum, item, i) => sum + (i === shelf ? depth : item.depth),
            0,
          ) + (current ? 0 : depth);
  ...
  return {
    length: Math.max(...shelves.map((shelf) => shelf.width)),
    width: shelves.reduce((sum, shelf) => sum + shelf.depth, 0),
  };
}
...
function pack(rectangles: Rect[]): { length: number; width: number } {
  ...
  const maxSide = Math.max(
    ...rectangles.map((item) => Math.min(item.length, item.width)),
  );
  const totalSide = rectangles.reduce(
    (sum, item) => sum + Math.max(item.length, item.width),
    0,
  );
  ...
  for (const order of orders)
    for (const limit of candidates) {
      ...
      if (result.length * result.width < best.length * best.width)
        best = result;
    }
  return best;
}

/** Builds exactly one non-vertically-stacked parcel for each stores. */
export function buildParcels(products: Product[]): Parcel[] {
  const groups = new Map<Id, Product[]>();
  for (const product of products)
    groups.set(product.storeId, [
      ...(groups.get(product.storeId) ?? []),
      product,
    ]);
    ...
    return {
      ...
      weight: round(
        storeProducts.reduce(
          (sum, product) => sum + product.weight * (product.quantity ?? 1),
          0,
        ),
      ),
    };
...
export async function priceDelivery(
  ...
  if (!String(customerPincode).trim())
    throw new TypeError("customerPincode is required");
  ...
  const quotes = await Promise.all(
    parcels.map(async (parcel) => {
      ...
      if (!store)
        throw new Error(`Store ${String(parcel.storeId)} was not provided`);
      const charge = await quote(parcel, store, customerPincode);
      if (!Number.isFinite(charge) || charge < 0)
        throw new RangeError(
          "Quote charge must be a non-negative finite number",
        );
  ...
  return {
    parcels,
    quotes,
    total: round(quotes.reduce((sum, item) => sum + item.charge, 0)),
  };
}
